...
The user rebooted and neither local service was running. The saved browser pairing and mappings still existed in the agent's filesystem view. A diagnostic launched by Windows Task Scheduler outside Codex then returned `ENOENT` for the same apparent `AppData\Local\OmniRouteSessionSync\installation.json` path and could not see the VBS launcher. The visible state inside Codex matched the physical file under `Packages\<PackageId>\LocalCache\Local\OmniRouteSessionSync`. Packaged-app AppData redirection made the earlier terminal-based startup check insufficient.
